...
// 20231213 Maria 使用edatagrid時，在 easyUI執行這行時，會跟boostrap5的tooltips事件打架(bootstrap叫做dispose)
// 如果之後有在使用到 easyUI的tooltip，應該還會延伸新的問題....
...
/*
    20241009 Maria 
    他這個驗證的判斷，如果是combogrid，她居然是抓 text的值，而不是 value的值???
    但如果 text給空字元 "　" ，它顯示就會變超奇怪啊!!所以我只能給全形空白空字元
    但這個判斷，當 opts.val(_59f) 會變成 "　"，就不會進去(WTF)
    反正他就是要空值，那我多個 trim給他不過分吧哈哈哈哈
*/
if(opts.val(_59f).trim()==""){
...
// 找到他底下的 textbox，加上變色
if ( tb.find(".textbox-text") !== undefined ){
    tb.find(".textbox-text").removeClass("focus-input");
}
...
// 找到他底下的 textbox，加上變色
if ( tb.find(".textbox-text") !== undefined ){
    tb.find(".textbox-text").addClass("focus-input");
}
...
// 20231106 Maria 添加新的 row屬性- numeric_format(欄位的顯示格式)
if(th.attr("numeric_format")){
var s1=$.trim(th.attr("numeric_format"));
if(s1.substr(0,1)=="{"){
col.numeric_format=eval("("+s1+")");
}else{
col.numeric_format=s1;
}
}
// 20231218 Maria 添加新的 row屬性- modal_name(分辨該資料位於哪個modal)
if(th.attr("modal_name")){
var s1=$.trim(th.attr("modal_name"));
if(s1.substr(0,1)=="{"){
col.modal_name=eval("("+s1+")");
}else{
col.modal_name=s1;
}
}
...
// 20240108 Maria 如果有設定 showPopUpComp 屬性，先把按鈕添加進去
if(opts.showPopUpComp){
    var PopMenuId = opts.id + "-methodList";
    var PopupComp = dc.header1.find('tr.datagrid-header-row .datagrid-header-rownumber')
    PopupComp.append('<Button class="btnPopupData" style="background-color: rgba(255,255,255,0);border:0px" ><span class="bi bi-grid-fill"  style="font-size:18px;color: #0d6efd;" onmouseenter="showPopMenu(this,\''+PopMenuId+'\')" onmouseout="hidePopMenu(this,\''+PopMenuId+'\')"  ></span></Button>');
}
if(opts.toolbar){
if($.isArray(opts.toolbar)){
...
// 20240601 Maria 讀取成功前，修正 rowNumber寬度
$(_839).datagrid("fixRownumber");
// 找到 datagrid底下目前已生成的 checkbox(但不包刮 全選 checkbox)
var $chkboxes = $(_839).parents().filter(".panel.datagrid").find(".datagrid-body").find("input[type='checkbox']");
...
$chkboxes.click(function(e) {
    ...
    if (e.shiftKey) {
        // 紀錄開始的 rowindex
        var tmp_start = $chkboxes.index(this);
        // 紀錄結束的 rowIndex
        var tmp_end = $chkboxes.index(lastChecked);
        // 比一下誰大誰小(因為可能會逆著回去)
        ...
        // 取得最後一次點擊 checkbox的是 勾選，還是 取消勾選，而tmp_end 是lastChecked
        var isChecked =$(_839).datagrid('getChecked').some(row => $(_839).datagrid('getRowIndex', row) === tmp_end);
        for ( let i = start ; i <= end; i++ ){
            if (isChecked){
                $(_839).datagrid("checkRow",i);
            }else{
                $(_839).datagrid("uncheckRow",i);
            }
        }
    }
    lastChecked = this;
...
// 20231226 Maria 如果有名為刪除的editor，自動略過不取
if(ed.field !== 'delete'){
_8a6.push(ed);
}}});
return _8a6;
...
if ( _8b7.length == 0){
    // 把行驗證搬到這邊來做
    var opts = $.data(_8b5,"edatagrid").options;
    var editIndex = opts.editIndex;
    var row = $(_8b5).datagrid("getRows")[editIndex];
    if ( opts.onValidRow && editIndex !== -1 ){
        var editors = $(_8b5).datagrid("getEditors",editIndex);
        $(editors).each(function(idx,editor){
            // 有focus先移除，才可以把 onChange做完
            var _8a0=$(editor.target).data("textbox")?$(editor.target).textbox("textbox"):$(editor.target);
            if(_8a0.is(":focus")){
                _8a0.triggerHandler("blur");
            }
            let newValue = editor.actions.getValue(editor.target)
            row[editor.field] =newValue; 
        })
        var continue_edit = opts.onValidRow(editIndex, row) == undefined ? true: opts.onValidRow(editIndex, row);
    }else{
        var continue_edit = true;
    }
}else{
    var continue_edit =  false;
}
...
// 20231225 Maria 如果editor是checkbox，上游的td加上click事件，對下游的checkbox進行checked(因為會冒泡，所以判斷是e.target是td才執行)
$(_909).bind('click',function(e){
if($(e.target).prop("nodeName") == "TD"){$(this).find(":checkbox").click();}})
...
return jq.each(function(){
    // 20240913 Maria 把按鍵事件移除綁定
    $(jq).datagrid('getPanel').panel('panel').attr('tabindex', 1).unbind('keydown');
...
/*
2023 Maria
showRow：透過指定idx，顯示整row
hideRow：透過指定idx，隱藏整row
*/
...
/*
    20240327 Maria 傳入新的欄位設定值，重設datagrid欄位值
    set的格式：
    [{field:欄位編碼,......}] <= field一定要給，其他則是column的屬性設定
    EX: 
    [
        {field:"status_approve_display",align:"right"},
        {field:"status_docclose_display",align:"right"},
        {field:"pdis_no_transout",formatter:testFormat},
    ];
*/
resetColOpts:function(jq,set){
    return jq.each(function () {
        for ( let i = 0 ; i < set.length ; i++ ){
            var field_new_set = set[i];
            var field = field_new_set.field == undefined?"":field_new_set.field;
            // 有設定欄位才繼續做
            if ( field !== ""){
                // 取得欄位目前的設定值
                var col_options = $(this).datagrid('getColumnOption',field);
                // 把新的設定給填上
                for (let key in field_new_set) {
                    // 20240506 Maria 如果是 editor的話，檢查是否為同一種元件
                    if ( key == 'editor' ){
                        // 同一個 editor元件的話 ，把屬性用加成的上去
                        if ( field_new_set[key]['type'] !== undefined ){
                            col_options[key] = field_new_set[key];
                        }else{
                            // 把原本的新屬性加上去
                            var editor_opts_n =field_new_set[key]['options']==undefined?{}:field_new_set[key]['options'];
                            for (let key_col in editor_opts_n) {
                                col_options[key]['options'][key_col] = editor_opts_n[key_col];
                            }
                        }
                    }else{  
                        col_options[key] = field_new_set[key];
...
/*
    20231227 Maria
    enablePopUpComp:修改為開啟Popup按鈕功能，如果PopUp按鈕不存在，就新增;若已存在，檢查是否隱藏，隱藏的話把它顯示
    disablePopUpComp:隱藏PopUpComp功能
*/
enablePopUpComp: function (jq) {
    //新增按鈕
    var opts = jq.datagrid('options');
    var header = jq.datagrid('getPanel').find('div.datagrid-header');
    var PopMenuId = opts.id + "-methodList";
    var PopupComp = header.find('tr.datagrid-header-row .datagrid-header-rownumber');
    if (PopupComp.find(".btnPopupData").length == 0) {
        var Popupbut = $('<Button class="btnPopupData" style="background-color: rgba(255,255,255,0);border:0px" ><span class="bi bi-grid-fill"  style="font-size:18px;color: #0d6efd;"  ></span></Button>');
        Popupbut.appendTo(PopupComp)
    } else {
        var Popupbut = PopupComp.find(".btnPopupData");
        $(Popupbut).css("visibility", "visible");
    }
    // 隱藏methodLiast
    $("#"+PopMenuId).removeClass("d-none");

},
disablePopUpComp:function(jq){
    var header = jq.datagrid('getPanel').find('div.datagrid-header');
    var opts = jq.datagrid('options');
    var PopMenuId = opts.id + "-methodList";
    var PopupComp = header.find('tr.datagrid-header-row .datagrid-header-rownumber');  
    var Popupbut = PopupComp.find(".btnPopupData");
    $(Popupbut).css("visibility","hidden");
    // 隱藏methodLiast
    $("#"+PopMenuId).addClass("d-none");
},
/*
    20231227 Maria
    hidePagination:隱藏分頁組件，包含分頁工具、頁碼、每頁筆數
    showPagination:顯示分頁組件，包含分頁工具、頁碼、每頁筆數
*/
hidePagination: function (jq) {
    // 取得分頁組件
    var pagination = jq.datagrid('getPager');
    pagination.find("table").addClass("invisible");
},
showPagination: function (jq) {
    // 取得分頁組件
    var pagination = jq.datagrid('getPager');
    pagination.find("table").removeClass("invisible");
},
/*
...
/*
    20240108 Maria 自行擴充 前端隱藏排序符號，並控制排序功能
    enableSort: 顯示排序符號，並開啟排序功能
    disableSort:隱藏排序符號，並鎖定排序功能
*/
enableSort:function(jq){
    return jq.each(function(){
        ...
        header.find('div.datagrid-sort').each(function(idx,val){
            $(val).find(".datagrid-sort-icon").removeClass("d-none");
        })
    })
},
disableSort: function(jq){
    return jq.each(function(){
        var state = $.data(this, 'datagrid');
        if( state == undefined ) return;
        var opts = state.options;
        ...
        var header = dc.header1.add(dc.header2);
        header.find('div.datagrid-sort').each(function(idx,val){
            $(val).find(".datagrid-sort-icon").addClass("d-none");
        })
    })
},};
...
var t=$(_946);
/* 
    20230108 Maria 新增 datagrid設定屬性 
    showPopUpComp：布林值，true為預設加入擴充菜單
    modal_name：字串，table隸屬的類型(main_modal/detail_modal_0/...)
    showFilter:布林值，true為預設加入過濾套件
*/
return $.extend({},$.fn.panel.parseOptions(_946),$.parser.parseOptions(_946,["url","toolbar","idField","sortName","sortOrder","pagePosition","resizeHandle",{sharedStyleSheet:"boolean",fitColumns:"boolean",autoRowHeight:"boolean",striped:"boolean",nowrap:"boolean",showPopUpComp:"boolean",showFilter:"boolean",},{rownumbers:"boolean",singleSelect:"boolean",ctrlSelect:"boolean",checkOnSelect:"boolean",selectOnCheck:"boolean"},{pagination:"boolean",pageSize:"number",pageNumber:"number"},{multiSort:"boolean",remoteSort:"boolean",showHeader:"boolean",showFooter:"boolean"},{scrollbarSize:"number",scrollOnSelect:"boolean"}]),{pageList:(t.attr("pageList")?eval(t.attr("pageList")):undefined),modalName:(t.attr("modalName")?t.attr("modalName"):undefined),loadMsg:(t.attr("loadMsg")!=undefined?t.attr("loadMsg"):undefined),rowStyler:(t.attr("rowStyler")?eval(t.attr("rowStyler")):undefined)});
...
// 20231123 Maria 為datagrid的td內容添加 title屬性(_961為欄位值)
// 如果是undefined，就顯示為空;如果欄位有formater，就顯示formater後的結果
if(_961==undefined){
cc.push("title='　' ");
}else{
// 不處理刪除跟checkbox
if(col.formatter && col.field !== "delete" && col.field !== "ck" ){
// 正常回傳值的formater不在此限，但會回傳元素的需要在這裡過濾
if (col.formatter.name !== "showBloean" ){
title = col.formatter(_961,_95e,_95d);
// 數字格式的話，辨識是否為數字
...
cc.push(">");
// 即使是 checkbox，也可以做 formatter判斷
if(col.checkbox){
    cc.push("<input type=\"checkbox\" "+(_95e.checked?"checked=\"checked\"":""));
    cc.push(" name=\""+_960+"\" value=\""+(_961!=undefined?_961:"\""));
    if(col.formatter){
        // 取得回傳結果 true跟false，true的話仍生成
        result = col.formatter(_961,_95e,_95d);
        if (!result){
            cc.push(" style=\"visibility: hidden;\" ");
        }
    }
    cc.push(">");
}else{
...
return $(_99b).datagrid("getRows");
    ...
    // console.log("start");
    var dg = $(this);
    var modal_name = $(this).attr("modalname") == undefined?"": $(this).attr("modalname");
    var row_count = $(this).datagrid("getRows");
    var prev_focus_editor_idx;
    /*---------------------------------------------------------------------------------------------*/
    // 開啟批次編輯狀態時，綁定keydown事件，可以透過鍵盤換行跟切換元件
    /*---------------------------------------------------------------------------------------------*/
    $(dg).datagrid('getPanel').panel('panel').attr('tabindex', 1).bind('keydown', function (e) {
        // 初始化要被編輯的row_index(如果沒有審核問題，通常也是從第一筆開始編輯)
        let edit_row_idx = 0;
        // 透過 edatagrid屬性 editIndex 取得目前為編輯狀態的 row_idx(如果有兩筆以上editrow狀況會失效)
        let selected_idx = $(dg).edatagrid('options').editIndex;
        // 取得該 row 的全部 editor 元件
        let editors = $(dg).datagrid('getEditors', selected_idx);
        // 初始化 focus_comp的idx位置(editors[idx])
        let focus_editor_idx = 0;
        // 進入時，都先取得按下按鍵當下，原本focus的元件位置
        $(editors).each(function (idx, val) {
            let target;
            target = $(val.target).data('textbox') ? $(val.target).textbox('textbox') : $(val.target);
            if (target.parent("span").hasClass("textbox-focused")) {
                prev_focus_editor_idx = idx;
            }
        })
        /*---------------------------------------------------------------------------------------------*/
        // 觸發keydown事件後，依照e.keyCode進行動作切換
        /* 
            [tab] 			下一個元件。新增：將後一個編輯元件變色；且當focus到當row最後一個元件時，回傳false不得在往後選
            [shift +tab]	上一個元件。新增：將前一個編輯元件變色；且當focus到當row第一個元件時，回傳false不得在往前選
            [38-方向鍵下]	下一個row。
            [40-方向鍵上]	上一個row
        */
        /*---------------------------------------------------------------------------------------------*/
        ...
        if ( e.keyCode == 9 && !e.shiftKey ) {
            i = prev_focus_editor_idx + 1;
            while ( i <= editors.length - 1 ){
                ...
                else if ( input_target.hasClass("custom_editor")){
                    i++;
                }
                else{
                    t = editors[i].target;
        ...
        /*---------------------------------------------------------------------------------------------*/
            ...
            let edit_row_idx = selected_idx;
            // 取得正在被選取的資料row的下一筆index
            let j = selected_idx - 1;
            // 跑while迴圈，直到找到下一筆【可以被修改】資料的row就跳出，並把datagrid滑到該筆所在位置，並select與開啟edit
            while (j >= 0) {
                let approved = CheckApproveStatus(modal_name,row_count[j]);
                if (!approved) {
                    edit_row_idx = j;
                    break;
                } 
            ...
            if ( edit_row_idx !==selected_idx ){
                $(dg).datagrid('scrollTo', edit_row_idx);
                // 選到要編輯的那筆
                $(dg).datagrid('selectRow', edit_row_idx);
                // 開始編輯該筆
                $(dg).edatagrid('editRow', edit_row_idx);
                // 編輯的時候如果editRow因為驗證失敗， editRow可能會偏移，所以改成選取 editRow以後處於編輯狀態得 editIndex
                var editIndex = $(dg).edatagrid('options').editIndex;
                // 取得新編輯行的所有editors，把focus指向正在編輯的focus_editor_idx，且將底色變色
                let new_editors = $(dg).datagrid('getEditors', editIndex);
                focus_editor_idx = prev_focus_editor_idx == undefined?0:prev_focus_editor_idx;
                let new_target = $(new_editors[focus_editor_idx].target).data('textbox') ? $(new_editors[focus_editor_idx].target).textbox('textbox') : $(new_editors[focus_editor_idx].target)
                // 如果要選取的 editor為鎖定/隱藏狀態，把焦點放到 datagrid的 panel上
                if( new_target.is(":disabled") ){
                    $(dg).datagrid('getPanel').panel('panel').focus();
                }else if( new_target.parents("td").is(":hidden")){
                    $(dg).datagrid('getPanel').panel('panel').focus();
                }else if ( new_target.hasClass("custom_editor")){
                    $(dg).datagrid('getPanel').panel('panel').focus();
                }
                else{
                    new_target.focus();
        ...
        /*---------------------------------------------------------------------------------------------*/
            ...
            let edit_row_idx = selected_idx;
            // 取得正在被選取的資料row的下一筆index
            let k = selected_idx+1;
            // 跑while迴圈，直到找到下一筆【可以被修改】資料的row就跳出，並把datagrid滑到該筆所在位置，並select與開啟edit
            while (k < row_count.length) {
                let approved = CheckApproveStatus(modal_name,row_count[k]);
                if (!approved) {
                    edit_row_idx = k;
                    break;
                } 
        ...
        // 20241014 Wallis 按enter後會觸發 sweetalert的對話框快速關掉。所以將預設的 enter按鍵事件取消
...
// 20230817 Maria 擴充combobox/combogrid功能，可以對該元件進行必填設定
...
// 20241101 Maria 不知道為甚麼 value的預設值會找不到，這邊補回去
var val = jq.combogrid("getValue") == undefined? "":jq.combogrid("getValue") ;
return $.extend($.data(jq[0],"combogrid").options,{value:val,width:_c68.width,height:_c68.height,originalValue:_c68.originalValue,disabled:_c68.disabled,readonly:_c68.readonly,editable:_c68.editable});
},cloneFrom:function(jq,from){
return jq.each(function(){
$(this).combo("cloneFrom",from);
...
// 20240601 Maria 新增 fixRowNumber寬度方法
...
// 20230512 Maria 取得工具列按鈕元素
function getPopUpComponent(target){
    //新增按鈕
    var header = $(target).datagrid('getPanel').find('div.datagrid-header');
    var PopupComp = header.find('tr.datagrid-header-row .datagrid-header-rownumber').parent("td");      
    return  PopupComp;     
}