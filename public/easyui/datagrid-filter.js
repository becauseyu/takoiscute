...
(function($){
	...
	var extendedOptions = {
		...
		// filterCache: {},
		filterExclude:[], //20230208 by leemin 可放不過濾欄位清單
		...
		defaultFilterOperator: 'like', // 20250115 Maria 初始化預設為 約等於 (LIKE %值%)
		defaultFilterTrigger: 'keyup', // 20230131 leemin 將原本keydown更改為 keyup 這樣才會讓input.val的值是最新的值
		defaultFilterOptions: {
			onInit: function(target){
				...
				input.off('.filter').on(trigger+'.filter', function(e){
					...
					// 20240124 Maria 修改為只有按下enter時才觸發，tab、清空都不觸發
					if (e.keyCode == 13){
						input.blur();                                                  
						_doFilter();
					} 
				...
				// 20240808 Maria input 改變時，觸發過濾(無法直接使用onChange是因為，如果使用者點取記憶的輸入值時，無法觸發)
				var start_val;         
				input.on('focus', function(e){
					start_val = this.value;
				})
	...
	// filter operators(remoteFilter = false 時使用)
	$.fn.datagrid.defaults.operators = {
		nofilter: {
			text: 'No Filter'
		},
		// 20250115 Maria  代表的是 SQL裡面的 in ()
		contains: {
			text: 'Include',
		...
		// 20250115 Maria 新增過濾方法，代表的是 sql的字串 LIKE
		like:{
			text: 'LIKE',
			isMatch: function(source, value){
				return source >= value;
		...
		// 20250115 Maria 新增過濾方法，代表的是 sql的字串 ~
		betweenDate:{
			text: 'Between',
			isMatch: function(source, value){
				return source >= value;
	...
	function init(target, filters){
		...
		// 20250116 Maria 如果datagrid有預設，就先用這個生成(不能直接放在 fiterRule去影響他)
		if (opts.filtertypeList !== undefined)filters =opts.filtertypeList
		...
		function createFilter(frozen){
			...
			for(var i=0; i<fields.length; i++){
				...
				if (field == '_'){
                    var but = $('<input type="button" id="btnFilterclear" style="background-color: rgba(255,255,255,0);border:0px;position:fixed" ><i class="bi bi-eraser-fill "  style="font-size:18px;color: SaddleBrown;margin:5px;cursor: pointer;" ></i>').appendTo(td)
                    //點擊item 輸出一個log text
                    but.on('click', function(){
						// 20250120 Maria 簡化成只對過濾器做 remove，後續讓 remove 自己去清空
						$(filters).each(function(idx,filter){
							removeFilterRule(target,filter.field);
						})
						doFilter(target);
                    });   
                    continue;
				...
				// 20250115 Maria  如果沒有另外給設定，預設值給 textbox
				else {
					fopts = $.extend({}, {
						field: field,
				...
				// 20230208 by  leemin 增加排除過濾項目
				var isExclude = opts.filterExclude.indexOf(field);
				...
				if (!div){
					if (isExclude<0){
						...
						if (fopts.op && fopts.op.length){
							...
							// 20250116 Maria OP會用於產生 filter旁邊的一個下拉選項
							if (fopts.options && fopts.options.onInit){
								fopts.options.onInit.call(input[0], target);
							} else if (fopts.defaultFilterOperator){
								opts.defaultFilterOptions.onInit.call(input[0], target);
		...
		function createFilterButton(container, operators){
			...
			// 20250117 Maria 預設從0開始
			$.map([''].concat(operators), function(item){
			...
			menu.menu({
				...
				onClick:function(item){
					...
					// 20250117 Maria 如果有驗證要先做
					if ( input.textbox("validate")){
			...
			// 20250117 Maria 首次創建好的時候，先把 LIKE 預設勾起來
			let like_item = $(menu).menu('findItems').filter(function(item){ return item.name === opts.defaultFilterOperator});
			$(menu).menu('setIcon', {
				target: like_item[0].target,
				iconCls: opts.filterMenuIconCls,
			});
	...
	$.extend($.fn.datagrid.methods, {
		...
		enableFilter: function(jq, filters){
			return jq.each(function(){
				// 20250115 Maria 他只要重複執行就會死掉，如果要重複執行先鎖定
				if ( $(this).datagrid("isFilterEnabled")){
		...
		resizeFilter: function(jq, field){
			return jq.each(function(){
		        ...
		        /*
            20231226 Maria 自行擴充，不改變原有過濾規則與相關設定，用於純粹前端鎖定filter
            lockFilter：鎖定過濾欄位，包含隱藏清除按鈕
            unlockFilter：開啟過濾欄位，包含顯示清除按鈕
        */
		lockFilter: function (jq) {
			return jq.each(function () {
				var name = getPluginName(this);
				var state = $.data(this, name);
				if ( state == undefined ){
				...
				var dc = $(this).data('datagrid').dc;
				var filter_text_box = opts.filterCache;
				for (var field in filter_text_box) {
					// 找到該欄位的filter-textbox
					let text_box = $(filter_text_box[field]).find(".datagrid-filter");
					// 如果是combobox，對元件進行鎖定
					if ( $(text_box).hasClass("combobox-f") ){
						$(text_box).combobox("disable")
					}
					// 新增日期區間欄位的判斷
					else if ($(text_box).prev("span").length >0 && $(text_box).parent("span").length >0){
						if (  $(text_box).parent("span").hasClass("datebox")){
							// 日期元件
							$(text_box).prev("span").hide();
							$(text_box).css("background-color","#f0f0f0");
						}
				...
				//隱藏橡皮擦(清除快篩功能)
				$(dc.header1).find("#btnFilterclear").next("i").addClass("invisible");
			});
		},
		unlockFilter: function (jq) {
			return jq.each(function () {
				var name = getPluginName(this);
				var state = $.data(this, name);
				if ( state == undefined ){
				...
				for (var field in filter_text_box) {
					// 比對欄位是否為初始設定的 非過濾欄位
					let filterExclude = opts.filterExclude;
					let filterable = filterExclude.filter(function (item) { return item == field; })
					// 確認不是不可過濾欄位，才打開
				...
				//隱藏橡皮擦(清除快篩功能)
				$(dc.header1).find("#btnFilterclear").next("i").removeClass("invisible");
			});
		},
	});
	// 20241118 Maria 擴充filter元件-區間datebox與datetimebox。元件文件:https://www.daterangepicker.com/ 
	$.extend($.fn.datagrid.defaults.filters, {
		dateRange: {
			init: function (container, options) {
				...
				var myoptions = {
					applyClass: 'btn-sm btn-success', // 確認的按鈕
					cancelClass: 'btn-sm btn-default', // 取消的按鈕
					autoUpdateInput: false, // 自動格式化 input，如果不打開的話，無法自動排列格式
					locale: {
						applyLabel: GetCompLang("easyui.search"),
						// applyLabel:EasyuiLang.search,
						cancelLabel: '清除',
						fromLabel: '開始時間',
						toLabel: '結束時間',
				...
				if (options.onChange == undefined) {
					...
					input.bind('keyup.filter', function(e){
						if (this.timer){
							clearTimeout(this.timer);
						}
						// 20240124 Maria 修改為只有按下enter時才觸發，tab、清空都不觸發
					...
					input.on('blur', function(e){
						...
						if ( start_val !== input_val){
							// 如果是清空，直接觸發
							if (this.value.trim() == ""){
								$(this).val('');
								options.onChange('');
							}else{
								var start = dateRangeParse(this.value).start;
								var end = dateRangeParse(this.value).end;
								if ( start =="" && end == ""){
									$(this).val('');
			...
			getValue: function (target) {
				return $(target).data('daterangepicker');
			},
			setValue: function (target, value) {
				if (value.trim() !== ""){
					var start = dateRangeParse(value).start;
					var end = dateRangeParse(value).end;
					$(target).daterangepicker({ startDate: start, endDate: end });
				}
			},
			resize: function (target, width) {
